...
此设计中，只有一个交互控件：文本输入框```Input_1```。有5个静态文本```Text_1```～```Text_5```，及两个静态的列表```List_1``` 和```List2```。
...
- 输入框的输入事件（一旦有输入就会触发）
  ...
  ```
  处理流程：
  1. 检查摆药单号的合法性（例如长度、数字等基本属性），检查通过才进行2
  2. 拿获取的摆药单号去查询明细（可能在此处访问数据库或缓存，也可能调用其他API服务）
  3. 如果获取到明细，继续4，如果失败，提示摆药单号错误
  4. 用查询到的数据更新Text文本和List列表的显示内容
  ```
...
目前在实现分布式架构时，主要采用B/S架构和前后端开发分离的方式。前端开发使用基于JS的框架，后端通过REST API提供服务，后端服务之间用RPC或REST进行服务的扩展。依赖不同编程语言的生态，各自有不同的框架可以选择，这里就不罗列。在设计时，需要考虑的几点：
...
- 工作流引擎的吞吐量。因为所有业务逻辑都要与工作流引擎进行交互，所以工作流引擎的吞吐量会成为潜在的流量瓶颈，这也是追求工作流引擎“轻量化”的目的之一。
